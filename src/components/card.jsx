...
function card({ steps, children, classes, cardName, subline, num, imgs ,cardIcon}) {
  const { cardsData } = useContext(globaldata);
  return (
    <section
      className={`${classes} backdrop-blur-[5px] bg-[rgba(255,_255,_255,_0.7)] text-[1.1rem] grid sm:grid-cols-1 relative overflow-hidden shadow-[0px_9px_25px_-21px_black] rounded-[18px] border border-[#0000003b] `} id={cardName}>
      {/* <div className="absolute inset-0 bg-red-400"></div> */}
      <button className="capitalize flex gap-2 items-center bg-[linear-gradient(0deg,_rgb(125,83,155),_rgba(0,0,0,1),_rgba(0,0,0,1),_rgb(170,189,205))] font-medium absolute top-[3px] z-[2] px-3 rounded-3xl p-1 left-[3px] bg-black text-white">
        {num}. {cardName} {cardIcon}
      </button>

      {/* scanner */}
      <div className="grid items-center grid-cols-[repeat(auto-fit,minmax(100px,1fr))] gap-2">
        <div className="grid items-center p-4 pt-[2.7rem]">
          {children}
        </div>
        <div className="p-2">
          {cardName.toLowerCase() === "menu" && (
            <img
              className="h-full drop-shadow-[4px_8px_14px_rgba(0,0,0,0.68)] max-h-40 object-contain w-full"
              src={imgs}
              alt={`${cardName} image`}
            />
          )}
          {cardName.toLowerCase() === "free wifi" && (
            <div>
              <img
                className="h-full drop-shadow-[2px_8px_14px_rgba(0,0,0,0.68)] max-h-40 w-full object-contain"
                src={imgs}
                alt="free wifi image"
              />
            </div>
          ...
          {cardName.toLowerCase() === "feedback" && (
            <img
              className="h-full drop-shadow-[4px_8px_14px_rgba(0,0,0,0.68)] object-contain w-full"
              src={imgs}
              alt={`${cardName} image`}
            />
          )}
          {cardName.toLowerCase() === "payment" && (
            <>
              <img
                className="h-full drop-shadow-[4px_8px_14px_rgba(0,0,0,0.68)] max-h-[12rem] object-contain w-full"
                src={imgs}
                alt={`${cardName} image`}
              />
            </>
